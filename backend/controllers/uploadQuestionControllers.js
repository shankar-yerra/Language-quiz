...
const uploadQuestion = expressAsyncHandler(async (req, res) => {
  // function to create and upload a question
  const {
    uid,
    lang_id,
    category,
  ...
  if (!isTeacher) {
    // if not a teacher then no permission
    res
      .status(403)
      .send(
  ...
  if (
    // required conditions to be filled to create a question
    !lang_id ||
    !category ||
    !desc ||
    !option1 ||
  ...
  try {
    ...
    if (isCreated) {
      // adding question if already created
      res.status(201).send("Question added");
    } else {
      // if no question is created
